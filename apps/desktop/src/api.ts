...
import { desktopHttpRequest, isTauriRuntime } from './desktop';
import { t } from './i18n';
...
async function requestJson<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  // Tauri WebView 访问本地 Tornado 时，application/json 容易触发 OPTIONS 预检。
  // 后端直接读取 request.body 解析 JSON，因此这里在有 body 时退回 safelisted Content-Type。
  ...
  try {
    ...
    let responseHeader = (_name: string): string | null => null;

    if (isTauriRuntime()) {
      const response = await desktopHttpRequest({
        url: requestUrl,
        method,
        headers,
        body: hasBody ? String(init?.body) : null,
      });
      if (!response) {
      ...
      responseHeader = (name: string): string | null => response.headers.get(name);
    }
    ...
    if (status < 200 || status >= 300) {
      ...
      try {
        if (responseContentType.includes('application/json')) {
          const errorBody = JSON.parse(responseBodyText) as {
            error_desc?: unknown;
            error_code?: unknown;
            message?: unknown;
          };
          ...
          const errorText = responseBodyText.trim();
          if (errorText) {
      ...
      // 鉴权失败：触发 token 输入
      if (status === 401 && (errorCode === 'auth_required' || errorCode === 'auth_invalid')) {
        showTokenDialog.value = true;
        throw new Error('Auth required');
      }

      const isProxyConnectionFailure = errorCode === 'BACKEND_UNAVAILABLE'
        || responseHeader('x-proxy-error') === 'backend-unavailable';
