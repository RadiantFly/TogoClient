...
fn candidate_togospace_roots() -> Vec<PathBuf> {
    ...
    candidates.push(PathBuf::from("/Volumes/Extend/git/TogoClient/external/TogoSpace"));
...
fn resolve_togospace_root() -> Option<PathBuf> {
    candidate_togospace_roots()
        .into_iter()
...
fn candidate_python_commands(root: &Path) -> Vec<String> {
    ...
    let path_candidates = [
        ...
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../../TogoSpace/.venv/bin/python"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../../TogoSpace/.venv/bin/python3"),
    ];
...
fn backend_pid(root: &Path) -> Option<u32> {
    let pid_file = root.join("run/backend.pid");
    let content = fs::read_to_string(pid_file).ok()?;
    content.trim().parse::<u32>().ok()
}

fn backend_running(root: &Path) -> bool {
    let Some(pid) = backend_pid(root) else {
        return false;
    };
...
fn desktop_status(app: &AppHandle) -> DesktopStatus {
    let togospace_root = resolve_togospace_root();
    let is_backend_running = togospace_root
        .as_deref()
        .map(backend_running)
        .unwrap_or(false);
    let backend_pid = togospace_root
        .as_deref()
...
fn backend_stdout_log(root: &Path) -> PathBuf {
    root.join("logs/backend_stdout.log")
}
...
fn prepare_backend_dirs(root: &Path) -> Result<(), String> {
    fs::create_dir_all(root.join("logs"))
        .map_err(|error| format!("无法创建日志目录 {}: {error}", root.join("logs").display()))?;
    fs::create_dir_all(root.join("run"))
        .map_err(|error| format!("无法创建运行目录 {}: {error}", root.join("run").display()))?;
    Ok(())
}
...
fn spawn_backend_process(root: &Path) -> Result<String, String> {
    prepare_backend_dirs(root)?;

    let log_path = backend_stdout_log(root);
    ...
    Err(format!(
        "未找到可用的 Python 解释器。请设置 TOGOSPACE_PYTHON，或为 external/TogoSpace 准备 .venv。\n{}",
        spawn_errors.join("\n")
    ))
}

fn stop_backend_process(root: &Path) -> Result<(), String> {
    let Some(pid) = backend_pid(root) else {
        return Ok(());
    };
    ...
    for _ in 0..10 {
        if !backend_running(root) {
            break;
        }
        std::thread::sleep(Duration::from_millis(300));
    }

    let _ = fs::remove_file(root.join("run/backend.pid"));
    Ok(())
}
...
fn start_togospace_backend(app: AppHandle) -> Result<DesktopStatus, String> {
    let root = resolve_togospace_root().ok_or("未找到 TogoSpace 根目录".to_string())?;
    ...
    let python_command = spawn_backend_process(&root)?;
    ...
    if status.backend_running {
        ...
        Err(format!(
            "已尝试使用 `{python_command}` 启动 external/TogoSpace，但未检测到运行中的后端进程。\n日志文件: {}\n{}",
            log_path.display(),
            log_excerpt
        ))
    }
